...
export async function POST(
    ...
    let rot = null;
        ...
        if (height !== undefined && height !== null && height !== '') {
            h = parseFloat(height);
            if (isNaN(h)) return NextResponse.json({ error: 'Invalid height' }, { status: 400 });
        }

        if (rotation !== undefined && rotation !== null && rotation !== '') {
            rot = parseFloat(rotation);
            if (isNaN(rot) || rot < 0 || rot > 360) return NextResponse.json({ error: 'Invalid rotation. Must be between 0 and 360' }, { status: 400 });
        }
