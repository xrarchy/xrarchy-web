-- Fix the foreign key column names to match API expectations

-- First, drop any policies that depend on project_assignments table
DROP POLICY IF EXISTS "projects_users_view_assigned" ON projects;

-- Now we can safely drop the existing project_assignments table
DROP TABLE IF EXISTS project_assignments CASCADE;

-- Recreate project_assignments table with correct column names for API
